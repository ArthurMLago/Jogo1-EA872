...
Player::Player() {
	...
	err = Pa_Initialize();
	if( err != paNoError ) {
		fprintf(stderr, "PortAudio error on Pa_Initialize: %d : %s\n", err, Pa_GetErrorText( err ) );
		return;
	}
...
Player::~Player() {
	...
	if( err != paNoError ) {
		fprintf(stderr, "PortAudio error on Pa_CloseStream: %d : %s\n", err, Pa_GetErrorText( err ) );
		return;
	}
...
int Player::play(Sample *audiosample, float *intensities){
    ...
    if (!playing){
        ...
        if( err != paNoError ) {
			fprintf(stderr, "PortAudio error on Pa_StartStream: %d : %s\n", err, Pa_GetErrorText( err ) );
